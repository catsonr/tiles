...
// matches stage; palette and region failures carry no prototile id. The debug
// region's outer boundary is an ordinary polygon, so its construction failure is
// reported through polygon_error at the region_outer_polygon stage.
...
// supply and the four quarter-turn orientations, paired with one temporary
// no-hole rectangular debug region and an empty arrangement.
//
